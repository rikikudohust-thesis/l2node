...
type PoolL2Tx struct {
	...
	Fee         FeeSelector
	Nonce       nonce.Nonce
	State       PoolL2TxState `gorm:"column:state"`
	MaxNumBatch uint32
  ...
  AbsoluteFeeUpdate time.Time `gorm:"-"`
	Type              TxType `gorm:"column:tx_type"`
	RqOffset          uint8
...
type PoolL2TxGorm struct {
	...
	RqToBJJ           babyjub.PublicKeyComp
	RqTokenID         TokenID
	RqAmount          string `gorm:"type:numeric"`
	RqFee             FeeSelector
	RqNonce           nonce.Nonce
	AbsoluteFee       float64
	AbsoluteFeeUpdate time.Time
	Type              TxType `gorm:"column:tx_type"`
	RqOffset          uint8
	// Extra DB write fields (not included in JSON)
	ClientIP string
...
func (tx *PoolL2TxGorm) ToPoolL2Tx() *PoolL2Tx {
	amountBI, _ := new(big.Int).SetString(tx.Amount, 10)
	rqAmountBI, _ := new(big.Int).SetString(tx.RqAmount, 10)
	return &PoolL2Tx{
		TxID:               tx.TxID,
		FromIdx:            tx.FromIdx,
		ToIdx:              tx.ToIdx,
		AuxToIdx:           tx.AuxToIdx,
		...
		RqFee:              tx.RqFee,
		RqNonce:            tx.RqNonce,
		AbsoluteFee:        tx.AbsoluteFee,
		AbsoluteFeeUpdate:  tx.AbsoluteFeeUpdate,
		Type:               tx.Type,
		RqOffset:           tx.RqOffset,
		ClientIP:           tx.ClientIP,
		RqTxCompressedData: tx.RqTxCompressedData,
...
func (tx *PoolL2Tx) RqTxCompressedDataV2() (*big.Int, error) {
	var amountFloat40 utils.Float40
	if tx.RqAmount == nil {
		af40, err := utils.NewFloat40(big.NewInt(0))
		if err != nil {
			return nil, tracerr.Wrap(err)
		}
		amountFloat40 = af40
	} else {
		af40, err := utils.NewFloat40(tx.RqAmount)
		if err != nil {
			return nil, tracerr.Wrap(err)
		}
		amountFloat40 = af40
...
func (tx *PoolL2Tx) HashToSign(chainID uint16) (*big.Int, error) {
	...
	rqToEthAddr := utils.EthAddrToBigInt(tx.RqToEthAddr)
