...
type L1TxGorm struct {
	...
	// EffectiveAmount only applies to L1UserTx.
	EffectiveAmount string `gorm:"type:numeric"`
	DepositAmount   string `gorm:"type:numeric"`
	// EffectiveDepositAmount only applies to L1UserTx.
	EffectiveDepositAmount string `gorm:"type:numeric"`
	// Ethereum Block Number in which this L1Tx was added to the queue
	EthBlockNum uint64
	EthTxHash   ethCommon.Hash
	L1Fee       string `gorm:"type:numeric"`
	Type        TxType
	BatchNum    *BatchNum
}

func (l1g *L1TxGorm) ToL1Tx() *L1Tx{
  amount := new(big.Int)
  amountBI, _ := amount.SetString(l1g.Amount, 10)
  eAmount := new(big.Int)
  eAmountBI, _ := eAmount.SetString(l1g.DepositAmount, 10)
  dAmount := new(big.Int)
  dAmountBI, _ := dAmount.SetString(l1g.DepositAmount, 10)
  edAmount := new(big.Int)
  edAmountBI, _ := edAmount.SetString(l1g.EffectiveDepositAmount, 10)
  fee := new(big.Int)
  feeBI, _ := fee.SetString(l1g.L1Fee, 10)

  return  &L1Tx{
    TxID: l1g.TxID,
    ToForgeL1TxsNum: l1g.ToForgeL1TxsNum,
    Position: l1g.Position,
    UserOrigin: l1g.UserOrigin,
    FromIdx: l1g.FromIdx,
    EffectiveFromIdx: l1g.EffectiveFromIdx,
    FromEthAddr: l1g.FromEthAddr,
    FromBJJ: l1g.FromBJJ,
    ToIdx: l1g.ToIdx,
    TokenID: l1g.TokenID,
    Amount: amountBI,
    EffectiveAmount: eAmountBI,
    DepositAmount: dAmountBI,
    EffectiveDepositAmount: edAmountBI,
    EthBlockNum: l1g.EthBlockNum,
    EthTxHash: l1g.EthTxHash,
    L1Fee: feeBI,
    Type: l1g.Type,
    BatchNum: l1g.BatchNum,
  }
}
...
func (tx *L1Tx) BytesDataAvailability(nLevels uint32) ([]byte, error) {
	...
	if tx.EffectiveAmount != nil {
		amountFloat40, err := utils.NewFloat40(tx.EffectiveAmount)
		if err != nil {
			return nil, tracerr.Wrap(err)
		}
		...
		copy(b[idxLen*2:idxLen*2+utils.Float40BytesLength], amountFloat40Bytes)
	}
	// fee = 0 (as is L1Tx)
	return b[:], nil
}
...
func L1TxFromDataAvailability(b []byte, nLevels uint32) (*L1Tx, error) {
	...
	amountBytes := b[idxLen*2 : idxLen*2+utils.Float40BytesLength]
	...
	l1tx.ToIdx = toIdx
	l1tx.EffectiveAmount, err = utils.Float40FromBytes(amountBytes).BigInt()
...
func (tx *L1Tx) BytesGeneric() ([]byte, error) {
	...
	if tx.FromBJJ != EmptyBJJComp {
		pkCompL := tx.FromBJJ
		pkCompB := utils.SwapEndianness(pkCompL[:])
	...
	depositAmountFloat40, err := utils.NewFloat40(tx.DepositAmount)
	if err != nil {
		return nil, tracerr.Wrap(err)
	}
	...
	amountFloat40, err := utils.NewFloat40(tx.Amount)
	if err != nil {
		return nil, tracerr.Wrap(err)
	}
	amountFloat40Bytes, err := amountFloat40.Bytes()
...
func L1UserTxFromBytes(b []byte) (*L1Tx, error) {
	...
	pkCompB := b[20:52]
	pkCompL := utils.SwapEndianness(pkCompB)
	...
	tx.FromIdx = fromIdx
	tx.DepositAmount, err = utils.Float40FromBytes(b[58:63]).BigInt()
	if err != nil {
		return nil, tracerr.Wrap(err)
	}
	tx.Amount, err = utils.Float40FromBytes(b[63:68]).BigInt()
	if err != nil {
		return nil, tracerr.Wrap(err)
	}
