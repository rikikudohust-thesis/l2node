...
type L2Tx struct {
	...
	BatchNum BatchNum `gorm:"column:batch_num"` // batchNum in which this tx was forged.
...
func (l2g *L2Gorm) ToL2Tx() *L2Tx {
  amount := new(big.Int)
  amountBI, _ := amount.SetString(l2g.Amount, 10)
  return &L2Tx{
    TxID: l2g.TxID,
    BatchNum: l2g.BatchNum,
    Position: l2g.Position,
    FromIdx: l2g.FromIdx,
    ToIdx: l2g.ToIdx,
    TokenID: l2g.TokenID,
    Amount: amountBI,
...
func (tx L2Tx) BytesDataAvailability(nLevels uint32) ([]byte, error) {
	...
	b[idxLen*2+utils.Float40BytesLength] = byte(tx.Fee)
...
func L2TxFromBytesDataAvailability(b []byte, nLevels int) (*L2Tx, error) {
	...
	tx.Amount, err = utils.Float40FromBytes(b[idxLen*2 : idxLen*2+utils.Float40BytesLength]).BigInt()
	if err != nil {
		return nil, tracerr.Wrap(err)
	}
	tx.Fee = FeeSelector(b[idxLen*2+utils.Float40BytesLength])
	return tx, nil
}
