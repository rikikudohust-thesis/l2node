...
type TxProcessor struct {
	...
	AccumulatedFees map[model.Idx]*big.Int
	...
	updatedAccounts map[model.Idx]*model.Account
	config          Config
}
...
type ProcessTxOutput struct {
	ZKInputs           *model.ZKInputs
	ExitInfos          []model.ExitInfo
	CreatedAccounts    []model.Account
	CoordinatorIdxsMap map[model.TokenID]model.Idx
	CollectedFees      map[model.TokenID]*big.Int
	// UpdatedAccounts returns the current state of each account
	// created/updated by any of the processed transactions.
	UpdatedAccounts map[model.Idx]*model.Account
}

func newErrorNotEnoughBalance(tx model.Tx) error {
	var msg error
	if tx.IsL1 {
...
func (txProcessor *TxProcessor) AccumulatedCoordFees() map[model.Idx]*big.Int {
...
// type==TypeBatchBuilder, returns the model.ZKInputs to generate the
...
// returns model.ExitTreeLeaf that is later used by the Synchronizer to update
// the HistoryDB, and adds Nonce & TokenID to the L2Txs.
// And if TypeSynchronizer returns an array of model.Account with all the
// created accounts.
func (txProcessor *TxProcessor) ProcessTxs(coordIdxs []model.Idx, l1usertxs, l1coordinatortxs []model.L1Tx,
	l2txs []model.PoolL2Tx) (ptOut *ProcessTxOutput, err error) {
	defer func() {
		if err == nil {
	...
	if txProcessor.state.Type() == statedb.TypeSynchronizer {
		txProcessor.updatedAccounts = make(map[model.Idx]*model.Account)
	}

	exits := make([]processedExit, nTx)

	if txProcessor.state.Type() == statedb.TypeBatchBuilder {
		txProcessor.zki = model.NewZKInputs(txProcessor.config.ChainID, txProcessor.config.MaxTx, txProcessor.config.MaxL1Tx,
	...
	// first occurrence)
	usedCoordTokenIDs := make(map[model.TokenID]bool)
	var filteredCoordIdxs []model.Idx
	...
	txProcessor.AccumulatedFees = make(map[model.Idx]*big.Int)
	...
	// TokenID
	var collectedFees map[model.TokenID]*big.Int
	if txProcessor.state.Type() == statedb.TypeSynchronizer || txProcessor.state.Type() == statedb.TypeBatchBuilder {
		collectedFees = make(map[model.TokenID]*big.Int)
	...
	if txProcessor.zki != nil {
		...
		txCompressedDataEmpty := model.TxCompressedDataEmpty(txProcessor.config.ChainID)
	...
	for _, idx := range coordIdxs {
		...
		if txProcessor.zki != nil {
			...
			txProcessor.zki.EthAddr3[indexFee] = utils.EthAddrToBigInt(accCoord.EthAddr)
		}
	...
	if txProcessor.state.Type() == statedb.TypeSynchronizer {
		...
		// generate model.ExitInfo data
		var exitInfos []model.ExitInfo
		var exitIdxs []model.Idx
		exitInfosByIdx := make(map[model.Idx]*model.ExitInfo)
		for i := 0; i < nTx; i++ {
			if !exits[i].exit {
				continue
			}
		...
		fmt.Printf("exittree: %+v\n", exitTree.Root().BigInt())
		proof, _ := exitTree.GenerateSCVerifierProof(big.NewInt(35), nil)
		fmt.Printf("exittree value: %+v\n", proof.Value.BigInt().String())
		fmt.Printf("exittree key: %+v\n", proof.Key.BigInt().String())
    for i := range proof.Siblings {
		  fmt.Printf("exittree siblings: %+v\n", proof.Siblings[i].BigInt().String())
    }
...
func (txProcessor *TxProcessor) getFeePlanTokens(coordIdxs []model.Idx) ([]*big.Int, error) {
	var tBI []*big.Int
	for i := 0; i < len(coordIdxs); i++ {
...
// And another *model.Account parameter which contains the created account in
// case that has been a new created account and that the StateDB is of type
// TypeSynchronizer.
func (txProcessor *TxProcessor) ProcessL1Tx(exitTree *merkletree.MerkleTree, tx *model.L1Tx) (*model.Idx,
	*model.Account, bool, *model.Account, error) {
	// ZKInputs
	if txProcessor.zki != nil {
		// Txs
		var err error
		...
		// L1Txs
		depositAmountF40, err := utils.NewFloat40(tx.DepositAmount)
		if err != nil {
			return nil, nil, false, nil, tracerr.Wrap(err)
		}
		txProcessor.zki.DepositAmountF[txProcessor.txIndex] = big.NewInt(int64(depositAmountF40))
		txProcessor.zki.FromEthAddr[txProcessor.txIndex] = utils.EthAddrToBigInt(tx.FromEthAddr)
		if tx.FromBJJ != model.EmptyBJJComp {
		...
		if tx.Type == model.TxTypeForceTransfer ||
			tx.Type == model.TxTypeDepositTransfer ||
			tx.Type == model.TxTypeCreateAccountDepositTransfer ||
			tx.Type == model.TxTypeForceExit {
			// in the cases where at L1Tx there is usage of the
			// Amount parameter, add it at the ZKInputs.AmountF
			// slot
			amountF40, err := utils.NewFloat40(tx.Amount)
			if err != nil {
				return nil, nil, false, nil, tracerr.Wrap(err)
			}
	...
	switch tx.Type {
	case model.TxTypeForceTransfer:
	...
	case model.TxTypeCreateAccountDeposit:
		txProcessor.computeEffectiveAmounts(tx)
	...
	case model.TxTypeDepositTransfer:
		txProcessor.computeEffectiveAmounts(tx)
	...
	case model.TxTypeCreateAccountDepositTransfer:
		txProcessor.computeEffectiveAmounts(tx)
	...
	if txProcessor.state.Type() == statedb.TypeSynchronizer &&
		(tx.Type == model.TxTypeCreateAccountDeposit ||
			tx.Type == model.TxTypeCreateAccountDepositTransfer) {
		var err error
...
// related to the Exit (in case of): Idx, ExitAccount, boolean determining if
// the Exit created a new Leaf in the ExitTree.
func (txProcessor *TxProcessor) ProcessL2Tx(coordIdxsMap map[model.TokenID]model.Idx,
	collectedFees map[model.TokenID]*big.Int, exitTree *merkletree.MerkleTree,
	tx *model.PoolL2Tx) (*model.Idx, *model.Account, bool, error) {
	var err error
	// if tx.ToIdx==0, get toIdx by ToEthAddr or ToBJJ
	if tx.ToIdx == model.Idx(0) && tx.AuxToIdx == model.Idx(0) {
		if txProcessor.state.Type() == statedb.TypeSynchronizer {
		...
		// case when tx.Type == model.TxTypeTransferToEthAddr or
		// model.TxTypeTransferToBJJ:
	...
	if txProcessor.zki != nil {
		...
		txProcessor.zki.ToEthAddr[txProcessor.txIndex] = utils.EthAddrToBigInt(tx.ToEthAddr)
		...
		if tx.RqOffset != 0 {
			...
			txProcessor.zki.RqToEthAddr[txProcessor.txIndex] = utils.EthAddrToBigInt(tx.RqToEthAddr)
		}
	...
	switch tx.Type {
	case model.TxTypeTransfer, model.TxTypeTransferToEthAddr, model.TxTypeTransferToBJJ:
...
func (txProcessor *TxProcessor) applyCreateAccount(tx *model.L1Tx) error {
	...
	p, err := txProcessor.createAccount(model.Idx(txProcessor.state.CurrentIdx()+1), account)
	if err != nil {
	...
	if txProcessor.zki != nil {
		...
		txProcessor.zki.EthAddr1[txProcessor.txIndex] = utils.EthAddrToBigInt(tx.FromEthAddr)
		...
		txProcessor.zki.AuxFromIdx[txProcessor.txIndex] = model.Idx(txProcessor.state.CurrentIdx() + 1).BigInt()
...
// of TypeSynchronizer
func (txProcessor *TxProcessor) createAccount(idx model.Idx, account *model.Account) (
...
// of TypeSynchronizer
func (txProcessor *TxProcessor) updateAccount(idx model.Idx, account *model.Account) (
	*merkletree.CircomProcessorProof, error) {
	if txProcessor.state.Type() == statedb.TypeSynchronizer {
		account.Idx = idx
		txProcessor.updatedAccounts[idx] = account
...
func (txProcessor *TxProcessor) applyDeposit(tx *model.L1Tx, transfer bool) error {
	accSender, err := txProcessor.state.GetAccount(tx.FromIdx)
	if err != nil {
		return tracerr.Wrap(err)
	}
	...
	if txProcessor.zki != nil {
		...
		txProcessor.zki.EthAddr1[txProcessor.txIndex] = utils.EthAddrToBigInt(accSender.EthAddr)
	}
	...
	if transfer {
		...
		if txProcessor.zki != nil {
			...
			txProcessor.zki.EthAddr2[txProcessor.txIndex] = utils.EthAddrToBigInt(accReceiver.EthAddr)
		}
...
func (txProcessor *TxProcessor) applyTransfer(coordIdxsMap map[model.TokenID]model.Idx,
	collectedFees map[model.TokenID]*big.Int, tx model.Tx, auxToIdx model.Idx) error {
	if auxToIdx == model.Idx(0) {
		auxToIdx = tx.ToIdx
	}
	...
	if !tx.IsL1 { // L2
		...
		fee, err := model.CalcFeeAmount(tx.Amount, *tx.Fee)
		if err != nil {
			return tracerr.Wrap(err)
		}
...
func (txProcessor *TxProcessor) applyCreateAccountDepositTransfer(tx *model.L1Tx) error {
	auxFromIdx := model.Idx(txProcessor.state.CurrentIdx() + 1)
	accSender := &model.Account{
		TokenID: tx.TokenID,
		Nonce:   0,
		Balance: tx.EffectiveDepositAmount,
		BJJ:     tx.FromBJJ,
	...
	p, err := txProcessor.createAccount(model.Idx(txProcessor.state.CurrentIdx()+1), accSender)
	if err != nil {
		return tracerr.Wrap(err)
	}
	if txProcessor.zki != nil {
...
func (txProcessor *TxProcessor) applyExit(coordIdxsMap map[model.TokenID]model.Idx,
	collectedFees map[model.TokenID]*big.Int, exitTree *merkletree.MerkleTree,
	tx model.Tx, originalAmount *big.Int) (*model.Account, bool, error) {
	...
	if txProcessor.zki != nil {
		...
		txProcessor.zki.EthAddr1[txProcessor.txIndex] = utils.EthAddrToBigInt(acc.EthAddr)
	}
	...
	if tracerr.Unwrap(err) == db.ErrNotFound {
		...
		exitAccount := &model.Account{
			TokenID: acc.TokenID,
			Nonce:   nonce.Nonce(0),
			// as is a model.Tx, the tx.Amount is already an
			// EffectiveAmount
			Balance: tx.Amount,
			BJJ:     acc.BJJ,
			EthAddr: acc.EthAddr,
		...
		if txProcessor.zki != nil {
			...
			txProcessor.zki.EthAddr2[txProcessor.txIndex] = utils.EthAddrToBigInt(acc.EthAddr)
...
func (txProcessor *TxProcessor) computeEffectiveAmounts(tx *model.L1Tx) {
	...
	if tx.Type == model.TxTypeCreateAccountDeposit {
		return
	}

	if tx.ToIdx >= model.UserThreshold && tx.FromIdx == model.Idx(0) {
	...
	if tx.ToIdx == model.Idx(1) || tx.ToIdx == model.Idx(0) {
...
func (txProcessor *TxProcessor) CheckEnoughBalance(tx model.PoolL2Tx) (bool, *big.Int, *big.Int) {
	acc, err := txProcessor.state.GetAccount(tx.FromIdx)
	if err != nil {
