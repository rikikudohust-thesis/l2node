...
func formatAccumulatedFees(collectedFees map[model.TokenID]*big.Int, orderTokenIDs []*big.Int,
	coordIdxs []model.Idx) []*big.Int {
	accFeeOut := make([]*big.Int, len(orderTokenIDs))
	for i := 0; i < len(accFeeOut); i++ {
		accFeeOut[i] = big.NewInt(0)
	}
	for i := 0; i < len(coordIdxs); i++ {
		tokenID := model.TokenIDFromBigInt(orderTokenIDs[i])
