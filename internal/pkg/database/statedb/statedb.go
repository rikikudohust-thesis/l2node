...
import (
  ...
  "github.com/rikikudohust-thesis/l2node/internal/pkg/database/kvdb"
...
func (s *Last) GetAccount(idx model.Idx) (*model.Account, error) {
	return GetAccountInTreeDB(s.db, idx)
}

// GetCurrentBatch returns the current BatchNum stored in Last.db
func (s *Last) GetCurrentBatch() (model.BatchNum, error) {
	...
	return model.BatchNumFromBytes(cbBytes)
}
...
// only.
func (s *Last) GetAccounts() ([]model.Account, error) {
...
func (s *StateDB) LastGetAccount(idx model.Idx) (*model.Account, error) {
...
func (s *StateDB) LastGetCurrentBatch() (model.BatchNum, error) {
	var batchNum model.BatchNum
	if err := s.LastRead(func(sdb *Last) error {
		var err error
...
func (s *StateDB) GetCurrentIdx() (model.Idx, error) {
	return s.db.GetCurrentIdx()
}

// SetCurrentIdx stores Idx in the StateDB
func (s *StateDB) SetCurrentIdx(idx model.Idx) error {
...
func (s *StateDB) Reset(batchNum model.BatchNum) error {
...
// GetAccount returns the account for the given Idx
func (s *StateDB) GetAccount(idx model.Idx) (*model.Account, error) {
	return GetAccountInTreeDB(s.db.DB(), idx)
}

func accountsIter(db db.Storage, fn func(a *model.Account) (bool, error)) error {
	idxDB := db.WithPrefix(PrefixKeyIdx)
	if err := idxDB.Iterate(func(k []byte, v []byte) (bool, error) {
		idx, err := model.IdxFromBytes(k)
		if err != nil {
...
func getAccounts(db db.Storage) ([]model.Account, error) {
	accs := []model.Account{}
	if err := accountsIter(
		db,
		func(a *model.Account) (bool, error) {
			accs = append(accs, *a)
			return true, nil
		},
	); err != nil {
...
func (s *StateDB) TestGetAccounts() ([]model.Account, error) {
...
func GetAccountInTreeDB(sto db.Storage, idx model.Idx) (*model.Account, error) {
	...
	var b [32 * model.NLeafElems]byte
	copy(b[:], accBytes)
	account, err := model.AccountFromBytes(b)
	if err != nil {
		return nil, tracerr.Wrap(err)
	}
	account.Idx = idx
...
func (s *StateDB) CreateAccount(idx model.Idx, account *model.Account) (
...
// StateDB.MT==nil, MerkleTree is not affected, otherwise updates the
// MerkleTree, returning a CircomProcessorProof.
func CreateAccountInTreeDB(sto db.Storage, mt *merkletree.MerkleTree, idx model.Idx,
	account *model.Account) (*merkletree.CircomProcessorProof, error) {
...
// StateDB.mt==nil, MerkleTree is not affected, otherwise updates the
// MerkleTree, returning a CircomProcessorProof.
func (s *StateDB) UpdateAccount(idx model.Idx, account *model.Account) (
	*merkletree.CircomProcessorProof, error) {
...
// StateDB.mt==nil, MerkleTree is not affected, otherwise updates the
// MerkleTree, returning a CircomProcessorProof.
func UpdateAccountInTreeDB(sto db.Storage, mt *merkletree.MerkleTree, idx model.Idx,
	account *model.Account) (*merkletree.CircomProcessorProof, error) {
...
func (s *StateDB) MTGetProof(idx model.Idx) (*merkletree.CircomVerifierProof, error) {
	if s.MT == nil {
...
func (l *LocalStateDB) CheckpointExists(batchNum model.BatchNum) (bool, error) {
...
func (l *LocalStateDB) Reset(batchNum model.BatchNum, fromSynchronizer bool) error {
	if fromSynchronizer {
