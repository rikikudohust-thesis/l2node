...
type KVDB struct {
	...
	CurrentIdx      model.Idx
	CurrentBatch    model.BatchNum
...
func (k *Last) set(kvdb *KVDB, batchNum model.BatchNum) error {
	k.rw.Lock()
	defer k.rw.Unlock()
	if k.db != nil {
		k.db.Close()
...
func (k *KVDB) Reset(batchNum model.BatchNum) error {
...
func (k *KVDB) reset(batchNum model.BatchNum, closeCurrent bool) error {
	...
	for ; start < len(list); start++ {
		if model.BatchNum(list[start]) > batchNum {
			break
		}
	}
	for _, bn := range list[start:] {
		if err := k.DeleteCheckpoint(model.BatchNum(bn)); err != nil {
			return tracerr.Wrap(err)
		}
	}
	...
	if batchNum == 0 {
		...
		k.db = sto
		k.CurrentIdx = model.RollupConstReservedIDx // 255
		k.CurrentBatch = 0
		if k.last != nil {
...
func (k *KVDB) ResetFromSynchronizer(batchNum model.BatchNum, synchronizerKVDB *KVDB) error {
...
func (k *KVDB) GetCurrentBatch() (model.BatchNum, error) {
...
func (k *KVDB) GetCurrentIdx() (model.Idx, error) {
	idxBytes, err := k.db.Get(keyCurrentIdx)
	if tracerr.Unwrap(err) == db.ErrNotFound {
		return model.RollupConstReservedIDx, nil // 255, nil
	}
	if err != nil {
		return 0, tracerr.Wrap(err)
	}
	return model.IdxFromBytes(idxBytes[:])
}

// SetCurrentIdx stores Idx in the KVDB
func (k *KVDB) SetCurrentIdx(idx model.Idx) error {
	k.CurrentIdx = idx
...
func (k *KVDB) CheckpointExists(batchNum model.BatchNum) (bool, error) {
...
func (k *KVDB) DeleteCheckpoint(batchNum model.BatchNum) error {
	checkpointPath := path.Join(k.cfg.Path, fmt.Sprintf("%s%d", PathBatchNum, batchNum))
...
func (k *KVDB) DeleteOldCheckpoints() error {
	...
	if k.cfg.Keep > 0 && len(list) > k.cfg.Keep {
		for _, checkpoint := range list[:len(list)-k.cfg.Keep] {
			if err := k.DeleteCheckpoint(model.BatchNum(checkpoint)); err != nil {
				return tracerr.Wrap(err)
			}
		}
	}
...
func (k *KVDB) MakeCheckpointFromTo(fromBatchNum model.BatchNum, dest string) error {
